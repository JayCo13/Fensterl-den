-- Run this in Supabase SQL Editor to update design names
-- This only updates the designs table data, not the schema
-- Your email/backoffice function uses quote_requests which references designs by ID, so it will still work

-- First, let's delete existing designs and insert new Figur designs
-- The quote_requests table has a foreign key, so we need to be careful

-- Step 1: Delete designs that are NOT referenced by quote_requests
DELETE FROM designs 
WHERE id NOT IN (SELECT DISTINCT design_id FROM quote_requests WHERE design_id IS NOT NULL);

-- Step 2: For designs that ARE referenced, we'll update their names to Figur names
-- This preserves the quote history

-- Step 3: Insert the new Figur designs for aluminum
INSERT INTO designs (name, material, description, base_price, sort_order) VALUES
  ...
  ('Figur 7', 'aluminum', 'Vollflächiges Design ohne Lamellen', 199, 8),
  ('Figur 7G', 'aluminum', 'Figur 7 mit Glasausschnitt', 259, 9),
...
-- Step 4: Insert the new Figur designs for wood
INSERT INTO designs (name, material, description, base_price, sort_order) VALUES
  ...
  ('Figur 7', 'wood', 'Vollflächiges Design ohne Lamellen', 269, 8),
  ('Figur 7G', 'wood', 'Figur 7 mit Glasausschnitt', 329, 9),
